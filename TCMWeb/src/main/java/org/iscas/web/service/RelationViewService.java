...
@Service
@Transactional
public class RelationViewService {
	@Autowired 
...
//			repository = bookDiseaseRelationDao;
			BookDiseaseRelation relation = bookDiseaseRelationDao.findByBaseEntityAndRelatedEntityAndPath(bookname, entityvalue, path);
			if(relation!=null||!"".equals(entityvalue)) {
				bookDiseaseRelationDao.delete(relation);
			}else {
				relation = new BookDiseaseRelation();
...
//			repository = bookDiseaseRelationDao;
			BookMaterialRelation relation = bookMaterialRelationDao.findByBaseEntityAndRelatedEntityAndPath(bookname, entityvalue, path);
			if(relation!=null||!"".equals(entityvalue)) {
				bookMaterialRelationDao.delete(relation);
			}else {
				relation = new BookMaterialRelation();
				relation.setBaseEntity(bookname);
				relation.setPath(path);
				relation.setPosition(0);
				bookMaterialRelationDao.delete(relation);
...
//			repository = bookMaterialRelationDao;
			...
			BookPrescriptionRelation relation = bookPrescriptionRelationDao.findByBaseEntityAndRelatedEntityAndPath(bookname, entityvalue, path);
			if(relation!=null||!"".equals(entityvalue)) {
				bookPrescriptionRelationDao.delete(relation);
			}else {
				relation = new BookPrescriptionRelation();
				relation.setBaseEntity(bookname);
				relation.setPath(path);
				relation.setPosition(0);
				bookPrescriptionRelationDao.delete(relation);
...
//			repository = bookExpertRelationDao;
			BookExpertRelation relation = bookExpertRelationDao.findByBaseEntityAndRelatedEntityAndPath(bookname, entityvalue, path);
			if(relation!=null||!"".equals(entityvalue)) {
				bookExpertRelationDao.delete(relation);
			}else {
				relation = new BookExpertRelation();
				relation.setBaseEntity(bookname);
				relation.setPath(path);
				relation.setPosition(0);
				bookExpertRelationDao.delete(relation);
...
//		Repository<? extends RelationShip, Long> repository = null;
		;
		if(target.equals(EntityRelationType.BookDiseaseRelation)) {
//			repository = bookDiseaseRelationDao;
			BookDiseaseRelation relation = bookDiseaseRelationDao.findByBaseEntityAndRelatedEntityAndPath(bookname, entityvalue, path);
			if(relation!=null||!"".equals(entityvalue)) {
...
//			System.out.println(relation.getId());
		...
		else if (EntityRelationType.BookMaterialRelation.equals(target)) {
//			repository = bookDiseaseRelationDao;
			BookMaterialRelation relation = bookMaterialRelationDao.findByBaseEntityAndRelatedEntityAndPath(bookname, entityvalue, path);
			if(relation!=null||!"".equals(entityvalue)) {
				relation.setRelatedEntity(newvalue);
...
//			repository = bookMaterialRelationDao;
		...
		else if (EntityRelationType.BookPrescriptionRelation.equals(target)) {
			BookPrescriptionRelation relation = bookPrescriptionRelationDao.findByBaseEntityAndRelatedEntityAndPath(bookname, entityvalue, path);
			if(relation!=null||!"".equals(entityvalue)) {
				relation.setRelatedEntity(newvalue);
...
//			repository = bookPrescriptionRelationDao;
		...
		else if (EntityRelationType.BookExpertRelation.equals(target)) {
//			repository = bookExpertRelationDao;
			BookExpertRelation relation = bookExpertRelationDao.findByBaseEntityAndRelatedEntityAndPath(bookname, entityvalue, path);
			if(relation!=null||!"".equals(entityvalue)) {
				relation.setRelatedEntity(newvalue);
...
//			System.out.println(diseaseDao.findByNameIn(list).size());
	...
	public RelationCollectionView GenerateAllBookView(String bookname) {
		RelationCollection rc = new RelationCollection();
...
//			List<Integer> poslist = new ArrayList<>();
			HashMap<String, String> nameHash = new HashMap<>();
			for(RelationShip rs : relationList) {
//				BookRelation br = (BookRelation) rs;
//				poslist.add(br.getPosition());
				list.add(rs.getRelatedEntity());
				nameHash.put(rs.getRelatedEntity(), null);
			}
