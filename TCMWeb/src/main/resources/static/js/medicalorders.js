...
require(['jquery','bootstrap','jquery_page'], function ($) {
	//----------------------------------------------
	var data_columns = ['id','sickId','nowDiagnosis','updateAt'];
	var action_columns = ['查看'];
	set_url('/medicalorders/get');
	...
	var myfunction = function() {
		$('td>div>span').click(function(e){
			...
			if($(this).text()=="查看") {
				var medicalorders_id = $(this).parent().parent().parent("tr ").attr("id");
				medicalorders_id = medicalorders_id.split('-')[2]
				console.log(medicalorders_id)
				console.log(curr_page)
				console.log(curr_size)
				
				data = {
					"medicalordersId":medicalorders_id,
				...
				$.get("/medicalorders/info.html", data );
				location.href = "/medicalorders/info.html?"+params;
