...
require(['jquery','bootstrap','jquery_page'], function ($) {
	//----------------------------------------------updateAt
	var data_columns = ['id','nameCn','reg','updateAt'];
	var action_columns = ['查看'];
	set_url('/naturalmedicine/get');
	...
	var myfunction = function() {
		$('td > div > a > span').click(function(e){
			...
			if($(this).text()=="查看") {
				var naturalmedicine_id = $(this).parent().parent().parent().parent("tr ").attr("id");
				//alert(naturalmedicine_id);
				naturalmedicine_id = naturalmedicine_id.split('-')[2]
				//alert(naturalmedicine_id);
				console.log(naturalmedicine_id)
				console.log(curr_page)
				console.log(curr_size)
				
				data = {
					"naturalmedicineId":naturalmedicine_id,
				...
				$.get("/naturalmedicine/info.html", data );
				location.href = "/naturalmedicine/info.html?"+params;
