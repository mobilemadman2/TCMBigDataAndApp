...
require(['jquery','bootstrap','jquery_page'], function ($) {
	//----------------------------------------------
	var data_columns = ['id','sickId','prescribeTxt','updateAt'];
	var action_columns = ['查看'];
	set_url('/inspectionresult/get');
	...
	var myfunction = function() {
		$('td>div>span').click(function(e){
			...
			if($(this).text()=="查看") {
				var inspectionresult_id = $(this).parent().parent().parent("tr ").attr("id");
				inspectionresult_id = inspectionresult_id.split('-')[2]
				console.log(inspectionresult_id)
				console.log(curr_page)
				console.log(curr_size)
				
				data = {
					"inspectionresultId":inspectionresult_id,
				...
				$.get("/inspectionresult/info.html", data );
				location.href = "/inspectionresult/info.html?"+params;
