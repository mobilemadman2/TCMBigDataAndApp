...
require(['jquery','bootstrap','jquery_page'], function ($) {
	//----------------------------------------------
	var data_columns = ['id','sickId','visitNumber','updateAt'];
	var action_columns = ['查看'];
	set_url('/mainrecord/get');
	...
	var myfunction = function() {
		$('td>div>span').click(function(e){
			...
			if($(this).text()=="查看") {
				var mainrecord_id = $(this).parent().parent().parent("tr ").attr("id");
				mainrecord_id = mainrecord_id.split('-')[2]
				console.log(mainrecord_id)
				console.log(curr_page)
				console.log(curr_size)
				
				data = {
					"mainrecordId":mainrecord_id,
				...
				$.get("/mainrecord/info.html", data );
				location.href = "/mainrecord/info.html?"+params;
