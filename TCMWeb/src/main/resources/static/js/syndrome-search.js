...
require(['jquery','bootstrap','jquery_page'], function ($) {
	//----------------------------------------------
	var data_columns = ['id','name','general_description','updateAt'];
	var data_columns_name = ['证候名','概述','更新时间','操作'];
	var action_columns = ['查看'];
	set_url('/syndrome/syndrome-search');
	set_data_columns(data_columns);
	set_action_columns(action_columns);
	
	$('#syndrome-search').click(function(e){
		console.log('syndrome-search')
		console.log('123number\t'+$('#page-number-id').text())
		console.log('123size\t'+$('#page-size-id').text())
