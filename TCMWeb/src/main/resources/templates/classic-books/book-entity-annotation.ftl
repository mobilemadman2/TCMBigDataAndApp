...
<h4>自动实体识别:</h4>
  <div ng-repeat="(entitykey, name) in autobookentities.views">{{ name }}:
...
<!--   ng-class={undefined:'label label-primary', true:'label label-danger'}[entity.addFlag] -->
  	<div ng-class="entity.addFlag==true?'label label-default':'label label-primary'" style="margin:3px;" 
...
<!--   	</div> -->
	...
	<div class="col-lg-6">
<h4>已标注实体：</h4>
  <div ng-repeat="(entitykey, name) in bookentities.views">{{ name }}:
  <div ng-repeat="entity in bookentities.relations[entitykey]" >
  	<div class="label label-primary" style="margin:3px;" 
    ...
    <span class="glyphicon glyphicon-ok" aria-hidden="true" ng-click="finishEntity(entitykey, entity)" style="cursor:pointer;"></span>
  	...
  	<!-- 新添加实体标签 -->
  	<div>
  	  	<div class="label label-primary" style="margin:3px;" id="{{ entitykey+'_newTag' }}" ng-if="(entitykey+'_new')!=editingEntity">新增
...
//     	console.log(response.data)
    		...
    		$scope.getBookRelations($scope.bookname)
...
// 		console.log(curr)
	...
	$scope.getBookRelations = function(bookname, path){
		//获取已生成实例
		$q.all({
			first:$http({
		        method: 'GET',
		        url: '/all-book-entities',
		        params: {
		        	'bookname': $scope.bookname,
		        }		        
		    }).then(function successCallback(response) {
